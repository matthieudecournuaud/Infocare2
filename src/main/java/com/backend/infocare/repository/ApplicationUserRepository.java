...
public interface ApplicationUserRepository extends ApplicationUserRepositoryWithBagRelationships, JpaRepository<ApplicationUser, Long> {
    @Query("select applicationUser from ApplicationUser applicationUser where applicationUser.user.login = ?#{authentication.name}")
    List<ApplicationUser> findByUserIsCurrentUser();
