...
public class ApplicationUserRepositoryWithBagRelationshipsImpl implements ApplicationUserRepositoryWithBagRelationships {
    ...
    public Optional<ApplicationUser> fetchBagRelationships(Optional<ApplicationUser> applicationUser) {
        return applicationUser.map(this::fetchTickets);
    }
    ...
    public List<ApplicationUser> fetchBagRelationships(List<ApplicationUser> applicationUsers) {
        return Optional.of(applicationUsers).map(this::fetchTickets).orElse(Collections.emptyList());
    }

    ApplicationUser fetchTickets(ApplicationUser result) {
        return entityManager
            .createQuery(
                "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.tickets where applicationUser.id = :id",
    ...
    List<ApplicationUser> fetchTickets(List<ApplicationUser> applicationUsers) {
        ...
        List<ApplicationUser> result = entityManager
            .createQuery(
                "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.tickets where applicationUser in :applicationUsers",
                ApplicationUser.class
            )
