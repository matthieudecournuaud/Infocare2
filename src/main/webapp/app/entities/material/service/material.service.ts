...
type RestOf<T extends IMaterial | NewMaterial> = Omit<T, 'purchaseDate' | 'warrantyEndDate' | 'lastMaintenanceDate'> & {
...
export class MaterialService {
  ...
  protected convertDateFromClient<T extends IMaterial | NewMaterial | PartialUpdateMaterial>(material: T): RestOf<T> {
    return {
      ...material,
      purchaseDate: material.purchaseDate?.format(DATE_FORMAT) ?? null,
      warrantyEndDate: material.warrantyEndDate?.format(DATE_FORMAT) ?? null,
      lastMaintenanceDate: material.lastMaintenanceDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMaterial: RestMaterial): IMaterial {
    return {
      ...restMaterial,
      purchaseDate: restMaterial.purchaseDate ? dayjs(restMaterial.purchaseDate) : undefined,
      warrantyEndDate: restMaterial.warrantyEndDate ? dayjs(restMaterial.warrantyEndDate) : undefined,
      lastMaintenanceDate: restMaterial.lastMaintenanceDate ? dayjs(restMaterial.lastMaintenanceDate) : undefined,
    };
  }
