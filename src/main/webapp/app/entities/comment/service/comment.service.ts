...
type RestOf<T extends IComment | NewComment> = Omit<T, 'editedAt'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      editedAt: comment.editedAt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      editedAt: restComment.editedAt ? dayjs(restComment.editedAt) : undefined,
    };
  }
