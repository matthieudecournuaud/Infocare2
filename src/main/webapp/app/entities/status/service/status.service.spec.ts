...
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../status.test-samples';

import { StatusService } from './status.service';

const requireRestSample: IStatus = {
  ...
  describe('Service methods', () => {
    ...
    describe('addStatusToCollectionIfMissing', () => {
      it('should add a Status to an empty array', () => {
        const status: IStatus = sampleWithRequiredData;
      ...
      it("should add a Status to an array that doesn't contain it", () => {
        const status: IStatus = sampleWithRequiredData;
        const statusCollection: IStatus[] = [sampleWithPartialData];
        expectedResult = service.addStatusToCollectionIfMissing(statusCollection, status);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(status);
      });
      ...
      it('should add only unique Status to an array', () => {
        const statusArray: IStatus[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const statusCollection: IStatus[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const status: IStatus = sampleWithRequiredData;
        const status2: IStatus = sampleWithPartialData;
    ...
    describe('compareStatus', () => {
      it('Should return true if both entities are null', () => {
        ...
        const compareResult = service.compareStatus(entity1, entity2);
      ...
      it('Should return false if one entity is null', () => {
        ...
        const compareResult1 = service.compareStatus(entity1, entity2);
        const compareResult2 = service.compareStatus(entity2, entity1);
